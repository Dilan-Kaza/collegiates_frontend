    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {sorted.slice(0, 3).map((post, i) => (
                <Link key={post.blog_id} to={`/news/${post.blog_id}`} className={`bg-off-white border border-gray-200 rounded-lg p-4 flex flex-col gap-2 h-48 overflow-hidden hover:border-primary transition-colors${i > 0 ? " hidden sm:flex" : ""}`}>
